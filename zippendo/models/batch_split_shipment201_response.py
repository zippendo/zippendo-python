...
class BatchSplitShipment201Response(BaseModel):
    """
    BatchSplitShipment201Response
    """ # noqa: E501
    original_shipment: CreateShipment201Response = Field(alias="originalShipment")
    new_shipments: List[CreateShipment201Response] = Field(description="Newly created shipments resulting from the split.", alias="newShipments")
    __properties: ClassVar[List[str]] = ["originalShipment", "newShipments"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.original_shipment:
            _dict['originalShipment'] = self.original_shipment.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each item in new_shipments (list)
        _items = []
        if self.new_shipments:
            for _item_new_shipments in self.new_shipments:
                if _item_new_shipments:
                    _items.append(_item_new_shipments.to_dict())
            _dict['newShipments'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "originalShipment": CreateShipment201Response.from_dict(obj["originalShipment"]) if obj.get("originalShipment") is not None else None,
            "newShipments": [CreateShipment201Response.from_dict(_item) for _item in obj["newShipments"]] if obj.get("newShipments") is not None else None
        })
        return _obj
