...
class UpdateCarrierRequest(BaseModel):
    ...
    """ # noqa: E501
    name: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(default=None, description="Carrier display name", json_schema_extra={"examples": ["GLS"]})
    carrier_slug: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(default=None, description="Carrier slug identifier", alias="carrierSlug", json_schema_extra={"examples": ["gls"]})
    config: Optional[Dict[str, ListCarriers200ResponseDataInnerConfigValue]] = Field(default=None, description="Carrier configuration (required and optional fields)", json_schema_extra={"examples": [{"customerNumber": "123456"}]})
    ...
    __properties: ClassVar[List[str]] = ["name", "carrierSlug", "config", "brandId"]
