...
class VerifyApiToken200Response(BaseModel):
    """
    VerifyApiToken200Response
    """ # noqa: E501
    valid: StrictBool = Field(description="Whether the token is valid", json_schema_extra={"examples": [True]})
    token_id: Optional[StrictStr] = Field(default=None, description="Token identifier", alias="tokenId", json_schema_extra={"examples": ["tok_6e2fa83ij9"]})
    user_id: Optional[StrictStr] = Field(default=None, description="User identifier the token belongs to", alias="userId", json_schema_extra={"examples": ["usr_9f3kd92ld0"]})
    org_id: Optional[StrictStr] = Field(default=None, description="Organization identifier the token belongs to", alias="orgId", json_schema_extra={"examples": ["org_4d8af01qw2"]})
    scopes: Optional[List[StrictStr]] = Field(default=None, description="Permission scopes granted by the token", json_schema_extra={"examples": [["read:shipments"]]})
    expires_at: Optional[StrictStr] = Field(default=None, description="Expiry timestamp (ISO 8601), null if it never expires", alias="expiresAt", json_schema_extra={"examples": ["2026-09-20T14:30:00.000Z"]})
    __properties: ClassVar[List[str]] = ["valid", "tokenId", "userId", "orgId", "scopes", "expiresAt"]
