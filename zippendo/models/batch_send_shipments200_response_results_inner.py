...
class BatchSendShipments200ResponseResultsInner(BaseModel):
    """
    BatchSendShipments200ResponseResultsInner
    """ # noqa: E501
    shipment_id: StrictStr = Field(description="The shipment this result refers to.", alias="shipmentId", json_schema_extra={"examples": ["shp_01H8XABC123"]})
    status: StrictStr = Field(description="`sent` when the carrier booked it, `failed` when the carrier or Zippendo rejected it, and `skipped` when the batch ran out of time before reaching it. A `skipped` shipment was never sent to the carrier and is safe to submit again.", json_schema_extra={"examples": ["sent"]})
    code: Optional[StrictStr] = Field(default=None, description="Canonical machine-readable error code, present when `status` is `failed` or `skipped`.", json_schema_extra={"examples": ["CARRIER_ERROR"]})
    message: Optional[StrictStr] = Field(default=None, description="Human-readable detail, present when `status` is `failed` or `skipped`.", json_schema_extra={"examples": ["Shipment must be in pending or error status to be sent"]})
    errors: Optional[List[SendShipment422ResponseErrorsInner]] = Field(default=None, description="Carrier-specific errors, present when the carrier rejected the booking.")
    __properties: ClassVar[List[str]] = ["shipmentId", "status", "code", "message", "errors"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['sent', 'failed', 'skipped']):
            raise ValueError("must be one of enum values ('sent', 'failed', 'skipped')")
        return value
