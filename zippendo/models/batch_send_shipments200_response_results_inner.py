...
class BatchSendShipments200ResponseResultsInner(BaseModel):
    ...
    status: StrictStr = Field(description="Whether this shipment was successfully booked with its carrier.", json_schema_extra={"examples": ["sent"]})
    code: Optional[StrictStr] = Field(default=None, description="Canonical machine-readable error code, present when `status` is `failed`.", json_schema_extra={"examples": ["CARRIER_ERROR"]})
    message: Optional[StrictStr] = Field(default=None, description="Human-readable failure detail, present when `status` is `failed`.", json_schema_extra={"examples": ["Shipment must be in pending or error status to be sent"]})
    ...
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['sent', 'failed']):
            raise ValueError("must be one of enum values ('sent', 'failed')")
        return value
